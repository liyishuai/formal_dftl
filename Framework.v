...
Definition fld_init : option flash_device :=
  match ftl_init with
      | None => None
      | Some f => Some (nand_init,f)
  end.
 
(* Definition fld_init : option flash_device := Some (nand_init,ftl_init). *)


Definition fld_run (fld: flash_device) (cmd: command) (fld': flash_device) (bh: behav) : Prop :=
  match cmd with 
    | cmd_read lpn => exists d, fld_read fld lpn = Some d /\ bh = bh_value d
    | cmd_write lpn d => fld_write fld lpn d = Some fld' /\ bh = bh_void
  end.
